...
updateSelectAllCheckbox();};document.addEventListener("DOMContentLoaded",function(){initializeCheckboxLogic({selectAllId:"select-all",checkboxClass:".instructor-checkbox",selectedCountId1:"selected-count-1",selectedCountId2:"selected-count-2",deleteButtonId:"remove-button",classroomIdsInputId:"instructor_ids",});});;document.addEventListener("DOMContentLoaded",function(){document.body.addEventListener("click",function(event){if(event.target.classList.contains("assignCourseButton")){const button=event.target;const programId=button.getAttribute("data-program");const instituteId=button.getAttribute("data-institute");const curriculumYear=button.getAttribute("data-curriculum");const semester=button.getAttribute("data-semester");const courseListContainer=document.getElementById('course-list-container');const noItemsMessage=document.getElementById('no-course-message');const searchInput=document.getElementById('search-courses');const removeAllButton=document.getElementById('remove-all-courses');const courseList=document.getElementById('course-list');let selectedCourses=new Set();function fetchCourses(){const url=`/api/courses/?curriculum_id=${curriculumYear}&institute_id=${instituteId}&program_id=${programId}&semester=${semester};
        console.log(url);
        fetch(url)
      ...
      function createCourseElement(course) {
        ...
        courseElement.innerHTML = `<div class="flex items-center w-full"><div class="flex-1 text-sm text-gray-800"><p><strong>${course.course_code}</strong><span class="text-gray-800">&nbsp(${course.course_description})</span></p></div><div class="check-icon ${isSelected ? '' : 'hidden'}"><svg class="shrink-0 size-4 text-primary-600"xmlns="http://www.w3.org/2000/svg"width="16"height="16"fill="currentColor"viewBox="0 0 16 16"><path d="M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425a.247.247 0 0 1 .02-.022Z"/></svg></div></div>`;

        courseElement.addEventListener('click', () => {
          if (selectedCourses.has(course.course_code)) {
            selectedCourses.delete(course.course_code);
          } else {
            selectedCourses.add(course.course_code);
          }
      ...
      function updateSelectedCourses() {
        ...
        if (selectedCourses.size > 0) {
          selectedCourses.forEach(courseCode => {
            const course = Array.from(courseList.children).find(el => el.textContent.includes(courseCode));
            ...
            selectedItem.innerHTML = `<div class="px-2 whitespace-nowrap text-sm font-medium text-gray-800">${courseCode}</div><div class="remove-item ms-2.5 inline-flex justify-center items-center size-5 rounded-full text-gray-800 bg-gray-200 hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-400 cursor-pointer"data-value="${courseCode}"><svg class="shrink-0 size-3"xmlns="http://www.w3.org/2000/svg"width="24"height="24"viewBox="0 0 24 24"fill="none"stroke="currentColor"stroke-width="2"stroke-linecap="round"stroke-linejoin="round"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg></div>`;
      ...
      function updateCourseElements() {
        courseList.querySelectorAll('.course-item').forEach(courseElement => {
          ...
          if (checkIcon) {
            checkIcon.classList.toggle('hidden', !selectedCourses.has(courseCode));
          }
        });
      }
      ...
      function updateButtonStates() {
        ...
        const removeAllDesign = `<svg xmlns="http://www.w3.org/2000/svg"width="16"height="16"fill="currentColor"class="bi bi-dash"viewBox="0 0 16 16"><path d="M4 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 4 8"/></svg>Remove all(<span id="remove-all-count">${selectedCount}</span>)`;if(selectedCount>0){removeAllButton.innerHTML=removeAllDesign;removeAllButton.disabled=false;}else{removeAllButton.innerHTML=removeAllDesign.replace(/(\(\d+\))/,'');removeAllButton.disabled=true;}}
