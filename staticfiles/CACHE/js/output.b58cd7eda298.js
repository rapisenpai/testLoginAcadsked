...
document.querySelectorAll('[id^="removeFieldButton-"]').forEach(button=>{attachRemoveEvent(button);});function attachRemoveEvent(button){button.addEventListener('click',function(){const row=this.closest('tr');const table=row.closest('tbody');row.remove();if(table.children.length===0){const header=table.closest('table').querySelector('.course-header');if(header){header.style.display='none';}}
