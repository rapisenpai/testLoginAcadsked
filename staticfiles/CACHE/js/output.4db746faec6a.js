document.addEventListener('DOMContentLoaded',function(){const yearLevelMapping={'First Year':1,'Second Year':2,'Third Year':3,'Fourth Year':4};const semesterMapping={'1st Semester':1,'2nd Semester':2,'3rd Semester':3};document.addEventListener('DOMContentLoaded',function(){console.log('DOM fully loaded and parsed');const addButton=document.getElementById('add-course-button');const tableBody=document.getElementById('courses-table-body');if(!addButton){console.error('Add button not found');return;}
if(!tableBody){console.error('Table body not found');return;}
addButton.addEventListener('click',function(){const yearLevel=this.getAttribute('data-year-level');const semester=this.getAttribute('data-semester');if(!yearLevel||!semester){console.error('Missing year level or semester attribute');return;}
