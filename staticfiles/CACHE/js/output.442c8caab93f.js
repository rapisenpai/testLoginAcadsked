const yearLevelMapping={'First Year':1,'Second Year':2,'Third Year':3,'Fourth Year':4};const semesterMapping={'1st Semester':1,'2nd Semester':2,'3rd Semester':3};document.querySelectorAll('[id^="addFieldButton-"]').forEach(button=>{button.addEventListener('click',function(){const yearLevelStr=this.getAttribute('data-year-level');const semesterStr=this.getAttribute('data-semester');const yearLevel=yearLevelMapping[yearLevelStr];const semester=semesterMapping[semesterStr];console.log('Year Level:',yearLevelStr,'Semester:',semesterStr);console.log('Year Level (int):',yearLevel,'Semester (int):',semester);const table=document.querySelector(`table[data-year-level="${yearLevelStr}"][data-semester="${semesterStr}"] tbody`);if(!table){console.error('Table not found for the specified year level and semester');return;}
const noCoursesRow=table.querySelector('.no-courses');if(noCoursesRow){noCoursesRow.remove();}
const newRow=document.createElement('tr');newRow.classList.add('course-field');newRow.innerHTML=`
      ...
      `;table.appendChild(newRow);console.log('New row added:',newRow);attachRemoveEvent(newRow.querySelector('[id^="removeFieldButton-"]'));});});function attachRemoveEvent(button){button.addEventListener('click',function(){this.closest('tr').remove();console.log('Row removed');});}
