...
const table=document.querySelector(`table[data-year-level="${yearLevelStr}"][data-semester="${semesterStr}"] tbody`);if(!table){console.error('Table not found for the specified year level and semester');return;}
...
document.querySelectorAll('[id^="removeFieldButton-"]').forEach(button=>{attachRemoveEvent(button);});document.getElementById('UpdateCourseSubmit').addEventListener('click',function(){document.getElementById('updateCoursesForm').submit();});;