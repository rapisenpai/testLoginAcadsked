...
function attachValidation(){document.querySelectorAll('[id^="laboratory-input-"]').forEach(function(input){input.addEventListener('input',function(){validateInput(this,0,9);});});document.querySelectorAll('[id^="lecture-input-"]').forEach(function(input){input.addEventListener('input',function(){validateInput(this,1,9);});});document.querySelectorAll('[id^="course-code-input-"]').forEach(function(input){input.addEventListener('input',function(){validateTextInput(this);});});document.querySelectorAll('[id^="course_description-input-"]').forEach(function(input){input.addEventListener('input',function(){validateTextInput(this);});});}
...
const newRow=document.createElement('tr');const rowNumber=table.querySelectorAll('tr').length+1;newRow.classList.add('course-field','bg-white');newRow.innerHTML=`
                    ...
                    <td>
                      <input type="text"  required  id="course_description-input-${semester}-${yearLevel}-${rowNumber}" maxlength="100"  autocomplete="off" name="course_description" class="no-spinner py-2 px-3 block w-full border-gray-200 rounded-lg text-sm focus:border-primary-500 focus:ring-primary-500" placeholder="Course Description">
                    </td>
                    <td>
