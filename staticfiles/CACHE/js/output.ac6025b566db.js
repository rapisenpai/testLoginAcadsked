const yearLevelMapping={'First Year':1,'Second Year':2,'Third Year':3,'Fourth Year':4};const semesterMapping={'1st Semester':1,'2nd Semester':2,'3rd Semester':3};document.querySelectorAll('[id^="addFieldButton-"]').forEach(button=>{button.addEventListener('click',function(){const yearLevelStr=this.getAttribute('data-year-level');const semesterStr=this.getAttribute('data-semester');const yearLevel=yearLevelMapping[yearLevelStr];const semester=semesterMapping[semesterStr];console.log(`Year Level: ${yearLevel}, Semester: ${semester}`);const table=document.querySelector(`table[data-year-level="${yearLevelStr}"][data-semester="${semesterStr}"] tbody`);const noCoursesRow=table.querySelector('.no-courses');if(noCoursesRow){noCoursesRow.remove();}
