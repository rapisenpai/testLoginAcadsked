...
document.querySelectorAll('[id^="removeFieldButton-"]').forEach(button=>{attachRemoveEvent(button);});window.addEventListener('beforeunload',function(e){if(isDirty&&!isFormSubmitting){e.preventDefault();e.returnValue='';}});document.querySelectorAll('form').forEach(form=>{form.addEventListener('submit',function(){isFormSubmitting=true;});});;