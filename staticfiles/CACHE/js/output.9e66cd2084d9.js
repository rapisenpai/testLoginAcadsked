function validateInput(input,min,max){const value=parseInt(input.value,10);if(input.type==='text'){if(input.value.trim()===""){input.classList.add('border-red-500','focus:border-red-500','focus:ring-red-500');input.classList.remove('border-gray-200','focus:border-primary-500','focus:ring-primary-500');}else{input.classList.remove('border-red-500','focus:border-red-500','focus:ring-red-500');input.classList.add('border-gray-200','focus:border-primary-500','focus:ring-primary-500');}}else{if(isNaN(value)||value<min||value>max||input.value.trim()===""){input.classList.add('border-red-500','focus:border-red-500','focus:ring-red-500');input.classList.remove('border-gray-200','focus:border-primary-500','focus:ring-primary-500');}else{input.classList.remove('border-red-500','focus:border-red-500','focus:ring-red-500');input.classList.add('border-gray-200','focus:border-primary-500','focus:ring-primary-500');}}}
function attachValidation(){document.querySelectorAll('[id^="laboratory-input-"]').forEach(function(input){input.addEventListener('input',function(){validateInput(this,0,9);});});document.querySelectorAll('[id^="lecture-input-"]').forEach(function(input){input.addEventListener('input',function(){validateInput(this,1,9);});});document.querySelectorAll('[id^="course-code-input-"]').forEach(function(input){input.addEventListener('input',function(){validateInput(this);});});document.querySelectorAll('[id^="course-description-input-"]').forEach(function(input){input.addEventListener('input',function(){validateInput(this);});});}
attachValidation();function validateAllInputs(){document.querySelectorAll('[id^="laboratory-input-"]').forEach(function(input){validateInput(input,0,9);});document.querySelectorAll('[id^="lecture-input-"]').forEach(function(input){validateInput(input,1,9);});document.querySelectorAll('[id^="course-code-input-"]').forEach(function(input){validateInput(input);});document.querySelectorAll('[id^="course-description-input-"]').forEach(function(input){validateInput(input);});}
document.querySelector('form').addEventListener('submit',function(event){validateAllInputs();if(document.querySelector('.border-red-500')){event.preventDefault();}});const yearLevelMapping={'First Year':1,'Second Year':2,'Third Year':3,'Fourth Year':4};const semesterMapping={'1st Semester':1,'2nd Semester':2,'3rd Semester':3};document.querySelectorAll('[id^="addFieldButton-"]').forEach(button=>{button.addEventListener('click',function(){const yearLevelStr=this.getAttribute('data-year-level');const semesterStr=this.getAttribute('data-semester');const yearLevel=yearLevelMapping[yearLevelStr];const semester=semesterMapping[semesterStr];const table=document.querySelector(`table[data-year-level="${yearLevelStr}"][data-semester="${semesterStr}"] tbody`);const noCoursesRow=table.querySelector('.no-courses');if(noCoursesRow){noCoursesRow.remove();}
