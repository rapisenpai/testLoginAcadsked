function validateInput(input,min,max){const value=parseInt(input.value,10);console.log(`Validating input: ${input.value}, Min: ${min}, Max: ${max}`);if(isNaN(value)||value<min||value>max){input.classList.add('border-red-500','focus:border-red-500','focus:ring-red-500');input.classList.remove('border-gray-200','focus:border-primary-500','focus:ring-primary-500');}else{input.classList.remove('border-red-500','focus:border-red-500','focus:ring-red-500');input.classList.add('border-gray-200','focus:border-primary-500','focus:ring-primary-500');}}
function attachValidation(){document.querySelectorAll('[id^="laboratory-input-"]').forEach(function(input){console.log('Found laboratory input:',input);input.addEventListener('input',function(){validateInput(this,0,9);});});document.querySelectorAll('[id^="lecture-input-"]').forEach(function(input){console.log('Found lecture input:',input);input.addEventListener('input',function(){validateInput(this,1,9);});});}
...
const newRow=document.createElement('tr');const rowNumber=table.querySelectorAll('tr').length+1;newRow.classList.add('course-field','bg-white');newRow.innerHTML=`
                    ...
                    <td>
                      <input type="text"  id="course-code-input-${semester}-${yearLevel}-${rowNumber}"    maxlength="10"  autocomplete="off" name="course_code" class="py-2 px-3 block w-full border-gray-200 rounded-lg text-sm focus:border-primary-500 focus:ring-primary-500" placeholder="Course Code">
                    </td>
                    <td>
                      <input type="text"   id="course_description-input-${semester}-${yearLevel}-${rowNumber}" maxlength="50"  autocomplete="off" name="course_description" class="no-spinner py-2 px-3 block w-full border-gray-200 rounded-lg text-sm focus:border-primary-500 focus:ring-primary-500" placeholder="Course Description">
                    </td>
                    <td>
                      <input type="number" id="lecture-input-${semester}-${yearLevel}-${rowNumber}"   autocomplete="off" name="lecture" class="no-spinner py-2 px-3 block w-full border-gray-200 rounded-lg text-sm focus:border-primary-500 focus:ring-primary-500" placeholder="Lecture" min="1" max="9">
                    </td>
                    <td>
                      <input type="number" id="laboratory-input-${semester}-${yearLevel}-${rowNumber}"  autocomplete="off" name="laboratory" class="no-spinner py-2 px-3 block w-full border-gray-200 rounded-lg text-sm focus:border-primary-500 focus:ring-primary-500" placeholder="Laboratory" min="0" max="9">
                    </td>
                    <td>
                    <input type="hidden" name="course_id" value="">
                    <input type="hidden" min="1" max="3" name="semester" value="${semester}">
                    <input type="hidden"  min="1" max="4"  name="year_level" value="${yearLevel}">
